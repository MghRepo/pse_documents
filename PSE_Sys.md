...
Un système de détection d'intrusion (IDS) est un appareil ou une application logicielle qui surveille un réseau ou des systèmes afin de cibler et de détecter des activités malveillantes ou des violations de politique de sécurité. Chaque activité d'intrusion ou de violation est généralement reporté soit à un administrateur ou dans un système centralisé de gestion d'événement et d'information de sécurité (SIEM). Un système SIEM combine les sorties de sources multiples et utilise des technique de filtrage d'alarme afin de distinguer une activité malveillante d'une fausse alarme.
